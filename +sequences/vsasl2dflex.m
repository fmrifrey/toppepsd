...
%   nframes: number of frames (slow dimension)
%   nshots: number of shots (echo trains) per frame
%   nechoes: number of echoes in echo train (fast dimension)
%
%   ndd: number of disdaq echoes to play at beginning of each echo train
%   TE: echo time (ms), use 'min' for minumum, default = 'min'
%   TR: repition time for GRE only (ms), use 'min' for minimum, 
%       default = 50
%   Tdelay: delay time at end of echo train (ms), default = 500
%   padtime: extra deadtime in each core for pulse gen (us), default = 1000
%
...
%       100
%
%   vsasl_vsType: VS module type ('DRHS', 'DRHT', 'BIR4', 'BIR8', 'FTVSI')
%   vsasl_Vcut: velocity cutoff (cm/s), default = 2
%   vsasl_bcomposite: flag to use composite refocusing pulse (FT-VSI
%       only), default = 0
%   vsasl_bsinc: flag to use sinc modulation (FT-VSI only), default = 0
%
...
arg.vsasl_vsType = 'none';
arg.vsasl_Vcut = 2;
arg.vsasl_bcomposite = 0;
arg.vsasl_bsinc = 0;
arg.vsasl_pld = 1000;
...
% Create VSASL pulse
if strcmpi(arg.vsasl_vsType,'none')
    vsasl_rf = zeros(100,1);
    vsasl_lbl_gz = zeros(100,1);
    vsasl_ctl_gz = zeros(100,1);
else
    [vsasl_rf, vsasl_lbl_gz, vsasl_ctl_gz] = psdutils.asl.genVSASL( ...
        arg.vsasl_vsType, ...
        arg.vsasl_Vcut, ...
        sys.maxRF, ...
        sys.maxGrad, ...
        ...
        0, ...
        0, ...
        arg.vsasl_bcomposite, ...
        arg.vsasl_bsinc);
end

% Write VSASL label pulse out to mod file
toppe.writemod(sys, ...
    'rf', vsasl_rf, ...
    'gz', vsasl_lbl_gz, ...
    'ofname', 'vsasllbl.mod');
...
dur_vsasl = length(toppe.readmod('vsasllbl.mod'))*sys.raster + arg.padtime; % us
...
coresfiletext = [ ...
    ...
    "2 3 0" % readout, delay
    "1 4" % crusher
    "2 5 0" % vsasl label, delay
    "2 6 0" % vsasl ctl, delay
    "1 0" % delay
    ];
...
modulesfiletext = [ ...
    ...
    sprintf("vsasllbl.mod %d 1 0 -1", dur_vsasl)
    sprintf("vsaslctl.mod %d 1 0 -1", dur_vsasl)
    ];
...
for framen = 1:arg.nframes
    for shotn = 1:arg.nshots
        ...
        if ~strcmpi(arg.vsasl_vsType,'none') && ~mod(framen,2) % even - label frame
            % Write VSASL label pulse to loop
            toppe.write2loop('vsasllbl.mod', sys, ...
                'echo', 1, ...
                'view', 1, ...
                'slice', 1, ...
                'version', 6, ...
            ...
            % Write PLD deadtime to loop
            toppe.write2loop('delay', sys, ...
                'textra', arg.vsasl_pld, ...
                'core', 5);
        elseif ~strcmpi(arg.vsasl_vsType,'none') % even - control frame
            % Write VSASL control pulse to loop
            toppe.write2loop('vsaslctl.mod', sys, ...
                'echo', 1, ...
                'view', 1, ...
                'slice', 1, ...
                'version', 6, ...
